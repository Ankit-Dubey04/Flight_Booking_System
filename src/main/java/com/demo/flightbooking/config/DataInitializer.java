...
public class DataInitializer {
    ...
    /**
     * Seeds the database with sample flights on first startup.
     * Only runs if the flights table is empty — safe to keep in production.
     */
    @Bean
    public CommandLineRunner initFlights(FlightRepository flightRepository) {
        return args -> {
            if (flightRepository.count() == 0) {
                ...
                // ─── Cancelled / Delayed (for testing filters) ─────────────
                ...
                System.out.println("ℹ️ DataInitializer: Flights table already has data — skipping seed.");
            }
        };
    }
}
